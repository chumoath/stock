...
for fund in funds:
    ...
    # 筛选每月15日后的第一个交易日
    fund_data_after_15th = fund_data[fund_data['Day'] >= 15]
    trade_days_after_15th = fund_data_after_15th.groupby(['Year', 'Month']).first().reset_index()
    # print(trade_days_after_15th)
    # 初始化年度收益
    annual_results = {}

    # 按年份计算收益
    for year in years:
        year_data = trade_days_after_15th[trade_days_after_15th['Year'] == year]
...
# 保存结果到 CSV 文件
# results.to_csv("Monthly_Investment_After_15th_Profit_Percentage_2020_2024.csv", index=True, encoding="utf-8-sig")
