...
for fund in funds:
    ...
    # 筛选每个月的第一个交易日
    first_trade_days = fund_data.groupby(['Year', 'Month']).first().reset_index()
    # 初始化年度收益
    annual_results = {}

    # 按年份计算收益
    for year in years:
        year_data = first_trade_days[first_trade_days['Year'] == year]
...
# 保存结果到 CSV 文件
# results.to_csv("Monthly_Investment_Fund_Profit_Percentage_2020_2024.csv", index=True, encoding="utf-8-sig")
