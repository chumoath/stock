...
for fund in funds:
    ...
    # 筛选每月第一个交易日
    fund_data['Month_Start'] = ~fund_data['Month'].eq(fund_data['Month'].shift(1))
    monthly_first_days = fund_data[fund_data['Month_Start']]
    # print(monthly_first_days)
    # 初始化四年收益
    quadrennial_results = {}

    # 按四年间隔计算收益
    for start_year, end_year in quadrennial_periods:
        period_data = monthly_first_days[
            (monthly_first_days['Year'] >= start_year) & (monthly_first_days['Year'] <= end_year)
        ]
...
# 保存结果到 CSV 文件
# results.to_csv("Fund_Quadrennial_Profit_Percentage_Monthly_First_Trading.csv", index=True, encoding="utf-8-sig")
