...
for fund in funds:
    ...
    first_trade_days = fund_data.groupby('Month_Group').first().reset_index()

    # 保存每日净值和购买点
    all_data[fund_name] = {"daily_data": fund_data, "buy_points": first_trade_days}
...
for fund_name, data in all_data.items():
    ...
    # 在购买点标记
    # plt.scatter(buy_points['Date'], buy_points['NAV'], s=20, label=f"{fund_name} 购买点", zorder=5)
    plt.scatter(buy_points['Date'], buy_points['NAV'], s=100, label=f"{fund_name} 购买点", zorder=5)

# 图形设置
plt.title("基金每日净值变化及购买点标记", fontsize=16)
